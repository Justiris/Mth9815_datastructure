...
// Exercise 2 Future class here

enum FutureIdType { EURODOLLARFUTURE, BONDFUTURE };

class Future : public Product
{
public:
  // Future ctor
  Future(string _productId, FutureIdType _futureIdType, date _maturityDate); // three features here
  Future();
...
private:
  string productId; // product identifier variable
  FutureIdType futureIdType; // future id type variable
  date maturityDate; // maturity date variable
  ProductType productType; // product type variable
};

Future::Future(string _productId, FutureIdType _futureIdType, date _maturityDate) : Product(_productId, FUTURE), futureIdType(_futureIdType), maturityDate(_maturityDate)
{
	//get error here if not using initialization list, do not know why..


  //productId = _productId;
  //futureIdType = _futureIdType;
  //maturityDate = _maturityDate;
}
...
{
  output << future.GetFutureIdType() << " " << future.GetMaturityDate();
  return output;
}

// Subclass EuroDollarFuture
...
EuroDollarFuture::EuroDollarFuture(string _productId, date _maturityDate) : Future(_productId, EURODOLLARFUTURE, _maturityDate){}

EuroDollarFuture::EuroDollarFuture() : Future(){}

ostream& operator<<(ostream &output, const EuroDollarFuture &euroDollarFuture)
{
  output << euroDollarFuture.GetFutureIdType() << " " << euroDollarFuture.GetMaturityDate();
  return output;
}
...
public:
	BondFuture(string _productId, date _maturityDate);
	BondFuture();
	friend ostream& operator<<(ostream &output, const BondFuture &bondFuture);
};

BondFuture::BondFuture(string _productId, date _maturityDate) : Future(_productId, BONDFUTURE, _maturityDate){}

BondFuture::BondFuture() : Future(){}

ostream& operator<<(ostream &output, const BondFuture &bondFuture)
{
  output << bondFuture.GetFutureIdType() << " " << bondFuture.GetMaturityDate();
  return output;
}
